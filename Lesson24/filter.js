...
const oddNumbers = numbersExercise1.filter((number) => number % 2 === 1);
...
const wordsWithA = wordsExercise2.filter((word) => word.toLowerCase().includes("a"));
...
const cheapBooks = booksExercise9.filter(book => {
	if (typeof book.price === 'number') {
		return book.price < 10;
	} else {
		const maybeNumber = parseInt(book.price);
